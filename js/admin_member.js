...
function loadData(x){
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      var sImg = "";
      var sEmpPhone = "-";
      if(doc.data().empPicture !== null) {
        sImg = '<img src="'+ doc.data().empPicture +'" style="width:30px;height:30px;">';
      }
      if(doc.data().empPhone !== undefined) {
        sEmpPhone = ''+doc.data().empPhone+'';
      }

      var dataSet = [sImg, doc.data().linename, doc.data().empID, doc.data().empName, doc.data().empBr, doc.data().lineID, sEmpPhone, "<div class='btn-t1 btn-add' id="+i+">คลิก</div>" , doc.id, i];
...
function ClickID(x,id) {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      Eid = doc.id;
      str += '<div><div style="padding:10px;color:#000000;font-size:13px;font-weight:600;">ขอสิทธิ์เข้าใช้ระบบงาน</div>';
      str += '<div><img src="'+ doc.data().empPicture +'" style="max-width:120px;"></div>';
      ...
      if(doc.data().statusconfirm==1) {
        str += '<div class="btn-t4">คุณได้รับอนุมัติแล้ว</div><div class="clr"></div>';
        str += '<div class="btn-t0" onclick="CancelRead(\''+ sid +'\')" style="width:160px;">ยกเลิกสิทธิ์</div><div class="clr"></div>';
        str += '<div class="btn-t0" onclick="DeleteRead(\''+ sid +'\')" style="width:160px;">ลบรายการ</div><div class="clr"></div>';
        str += '<div class="btn-t0" onclick="CloseAll()" style="width:160px;">ปิดหน้าต่าง</div>';
      } else if(doc.data().statusconfirm==9) {
        str += '<div class="btn-t4">คุณไม่ได้รับอนุมัติ</div><div class="clr"></div>';
        str += '<div class="btn-t0" onclick="ConfirmRead(\''+ sid +'\')" style="width:160px;">อนุมัติสิทธิ์</div><div class="clr"></div>';
        str += '<div class="btn-t4" onclick="DeleteRead(\''+ sid +'\')" style="width:160px;">ลบรายการ</div><div class="clr"></div>';
        str += '<div class="btn-t0" onclick="CloseAll()" style="width:160px;">ปิดหน้าต่าง</div>';
      } else if(doc.data().statusconfirm==2) {
        str += '<div class="btn-t4">รอการอนุมัติ</div><div class="clr"></div>';
        str += '<div class="btn-t1" onclick="ConfirmRead(\''+ sid +'\')" style="width:160px;">อนุมัติ</div><div class="clr"></div>';
        str += '<div class="btn-t3" onclick="CancelRead(\''+ sid +'\')" style="width:160px;">ไม่อนุมัติ</div><div class="clr"></div>';
        str += '<div class="btn-t4" onclick="DeleteRead(\''+ sid +'\')" style="width:160px;">ลบรายการ</div><div class="clr"></div>';
        str += '<div class="btn-t2" onclick="CloseAll()" style="width:160px;">ปิดหน้าต่าง</div>';
      }
